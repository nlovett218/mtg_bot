          ...
          await forEach(chosen_target, async (target) => {
            ...
            if (targetCardFromLibrary.type.toLowerCase().includes('enchantment'))
            {
              ...
              if (copy_cardType == "aura_opponent") {
                ...
                var random_opponent = await Constants.getRandomOpponent(obj.id, obj.message.channel.guild.id, copy_permanentType, copy_targetSource);
...
var local = {
  ...
  getopponent:async function(cmd, args)
  {
    ...
    var randomOpponent = await Constants.getRandomOpponent(msgOwnerID, cmd.channel.guild.id, "creature", ["battlefield"]);
