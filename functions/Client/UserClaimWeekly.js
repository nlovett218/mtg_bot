...
var local = {
  claimweeklypack:async function(cmd, args) {
  ...
  afterCheckUser:async function(obj)
  {
    ...
    var canClaim = false;
    var packAmountToReceive = userDataObj.mtg_accountType.includes('ALPHA_TESTER') || userDataObj.mtg_accountType.includes('BOT_ADMIN') ? Constants.extraWeeklyPackAward : Constants.baseWeeklyPackAward;
    if (userDataObj.mtg_weeklyPackCooldown == null || userDataObj.mtg_weeklyPackCooldown == undefined)
    {
      ...
      var timeSinceLastClaim = new Date(mtg_weeklyPackCooldown);
      var timeToAllowClaim = Date.parse(new Date(Constants.moment(timeSinceLastClaim).add(Constants.weeklyCooldown, 'minutes').format(Constants.momentTimeFormat)));//new Date(Constants.moment(timeSinceLastDraw).add(10, 'minutes').format("YYYY-MM-DD h:mm:ss"));
      var now = Date.now();
      ...
      if (timeToAllowClaim <= now) {
        ...
        obj.message.reply(` you still have another \`${timeString}\` until you can claim your weekly pack(s)!`);
        canClaim = false;
      }
    }
    ...
    if (canClaim) {
      ...
      var updateUserDataQuery = `UPDATE mtg_user SET mtg_weeklyPackCooldown=STR_TO_DATE('${Constants.moment(now).format(Constants.momentTimeFormat)}', '%m-%d-%Y %H:%i:%s'), mtg_packs='${packs_update}' WHERE mtg_userID='${obj.id}'`;

      await HandleConnection.callDBFunction('MYSQL-fireAndForget', updateUserDataQuery);
      await obj.message.channel.send(`<@${obj.id}> -> you successfully claimed your ${packAmountToReceive} weekly pack(s)!`);
...
HandleFunctionCall.RegisterFunction(["weekly", "week", "wkly", "w", "claimweekly"], local.claimweeklypack);
