...
async def info(ctx: commands.Context, *args):
    ...
    try:
            ...
            #__type__ = str(args[1])
            #if (__type__ == "weapon"):
                #weapon_category_count = len(list_weapons)
                #str_list_weapons = ""
                #for key,value in list_weapons.items():
                    #str_list_weapons += key + ";"
                    #str_list_weapons += key + ": Value:" + value + "\n\n"
                    #for key2,value2 in key.items():
                        #str_list_weapons += key2 + ": Value:" + value + "\n"
                #await client.send_message(ctx.message.channel, msg + str(weapon_category_count) + " weapon type(s):\n" + str_list_weapons)
                #return
            #elif (__type__ == "ammunition"):
                #ammunition_count = len(list_ammunition)
                #str_list_ammunition = ""
                #for key,value in list_ammunition.items():
                    #str_list_ammunition += key + "\n"
                    #str_list_weapons += key + ": Value:" + value + "\n\n"
                    #for key2,value2 in key.items():
                        #str_list_weapons += key2 + ": Value:" + value + "\n"
                #await client.send_message(ctx.message.channel, msg + str(ammunition_count) + " ammunition type(s):\n" + str_list_ammunition)
                #return
            #elif (__type__ == "loot_chance"):
                #return
            #else:
                #await client.send_message(ctx.message.channel, msg + str(ERROR_CODES.UNKNOWN_COMMAND.value))
                #return
    except Error as e :
            print(e)
    finally:
